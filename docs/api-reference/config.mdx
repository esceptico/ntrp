...
### List chat/explore/memory models
...
Returns models grouped by provider, including Claude OAuth models when configured.
...
Returns all providers (Anthropic, OpenAI, Google, OpenRouter, Claude OAuth, Custom) with connection status, available models, and masked API key hints.
...
### Connect via Claude OAuth

```bash
curl -X POST http://localhost:6877/providers/anthropic/oauth \
  -H "Authorization: Bearer $API_KEY"
```

Opens a browser window for PKCE-based OAuth login with your Claude Pro/Max subscription.
