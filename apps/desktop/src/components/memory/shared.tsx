...
export function ListColumn<T>({
  ...
  return (
    <>
      ...
      {totalLabel && (
        <div className="px-4 py-2 text-[12px] text-faint tabular-nums">{totalLabel}</div>
      )}
    </>
  );
}
...
export function ListError({ title, message }: { title: string; message: string }) {
  return (
    <div className="rounded-[10px] border border-[rgba(184,68,43,0.16)] bg-bad-soft px-3 py-2.5">
      <div className="text-[13px] font-semibold text-bad">{title}</div>
      <div className="mt-0.5 text-[13px] leading-[1.4] text-[#8a3220]">{message}</div>
    </div>
  );
}
...
export function DetailMeta({ children }: { children: ReactNode }) {
  return (
    <div className="flex items-center flex-wrap gap-2 text-[12.5px] text-faint">{children}</div>
  );
}
...
export function MetaGrid({ rows }: { rows: (MetaGridRow | null | false)[] }) {
  ...
  return (
    <dl className="grid grid-cols-[110px_minmax(0,1fr)] gap-y-2.5 text-[13px]">
      {present.map((row) => (
    ...
    <>
      ...
      <dd
        className={clsx(
          ...
          row.mono && "font-mono text-[12.5px] break-all whitespace-pre-wrap",
        )}
      >
        {row.value}
      </dd>
...
export function DetailPlaceholder({ children }: { children: ReactNode }) {
  return (
    <div className="grid place-items-center h-full text-[14px] italic text-faint">{children}</div>
  );
}
...
export function ErrorPill({ message }: { message: string }) {
  return (
    <span
      className="mr-auto inline-flex items-center max-w-[60%] truncate rounded-md bg-bad-soft border border-[rgba(184,68,43,0.18)] text-bad text-[12.5px] px-2 py-[3px]"
      title={message}
    >
      {message}
    </span>
...
export function Pill({
  ...
  return (
    <span className={clsx("inline-flex items-center rounded-md border px-2 py-[3px] text-[12px]", classes)}>
      {children}
    </span>
  );
}
...
export function JsonBlock({ value }: { value: unknown }) {
  return (
    <pre className="m-0 max-h-[260px] overflow-auto scroll-thin rounded-[8px] border border-line-soft bg-code-bg px-3 py-2 text-[12.5px] leading-relaxed text-ink-soft whitespace-pre-wrap">
...
export function PrimaryBtn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex items-center h-7 px-3 rounded-md bg-ink text-on-ink text-[13px] font-medium tracking-[-0.005em] hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
    >
      {children}
    </button>
  );
...
export function GhostBtn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md text-[13px] text-ink-soft hover:bg-surface-soft hover:text-ink transition-colors disabled:opacity-50"
    >
      {children}
    </button>
  );
...
export function DangerBtn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md text-[13px] text-ink-soft hover:bg-[rgba(220,38,38,0.08)] hover:text-[#b42318] transition-colors disabled:opacity-50"
    >
      {children}
    </button>
  );
  ...
  return (
    <div className="relative flex-1 min-w-0">
      ...
      <input
        ...
        spellCheck={false}
        className="w-full h-7 pl-7 pr-2 rounded-md bg-[rgba(0,0,0,0.04)] focus:bg-[rgba(0,0,0,0.06)] border border-transparent focus:border-line-soft text-[13px] text-ink-soft placeholder:text-faint outline-none transition-[background-color,border-color]"
      />
    </div>
  );
}
