...
class MemoryService:
    ...
    async def profile_details(self, entry_id: int) -> tuple[ProfileEntry, list[Fact], list[Observation]]:
        ...
        observations = [
            observations_by_id[obs_id] for obs_id in entry.source_observation_ids if obs_id in observations_by_id
        ]
    ...
    async def _validate_profile_sources(
        ...
        missing_observation_ids = [
            observation_id for observation_id in source_observation_ids if observation_id not in observations_by_id
        ]
        if missing_fact_ids or missing_observation_ids:
            raise ValueError(f"Missing profile sources: facts={missing_fact_ids}, patterns={missing_observation_ids}")
