    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="text-sm font-medium leading-snug">{event.action.replaceAll("_", " ")}</div>
          <div className="mt-1 text-xs text-faint">{event.target_type}{event.target_id ? ` #${event.target_id}` : ""}</div>
        </div>
        <Pill>{event.actor}</Pill>
      </div>
      <div className="mt-1 text-xs text-faint">{formatRelativePast(event.created_at)}</div>
    </button>
  );
}
  ...
  return (
    <div className="px-7 py-6">
      <div className="mb-5 flex items-center gap-2">
        <h3 className="m-0 text-lg font-semibold tracking-[-0.01em] text-ink">{event.action.replaceAll("_", " ")}</h3>
        <Pill>{event.actor}</Pill>
      </div>
