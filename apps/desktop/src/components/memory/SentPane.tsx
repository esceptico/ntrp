    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="text-sm font-medium leading-snug">{sourceLabel(event.source)}</div>
          <div className="mt-1 line-clamp-2 text-xs leading-snug text-faint">{event.query || "no query"}</div>
        </div>
      </div>
    ...
    <div className="px-7 py-6">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="m-0 text-lg font-semibold tracking-[-0.01em] text-ink">{sourceLabel(event.source)}</h3>
          </div>
          <p className="m-0 mt-1 text-sm text-faint">{formatAbs(event.created_at)} · {event.policy_version}</p>
        </div>
      ...
      {event.query && (
        <section className="mb-5">
          <h4 className="m-0 mb-2 text-2xs font-semibold uppercase tracking-[0.08em] text-faint">Query</h4>
          <p className="m-0 rounded-[8px] border border-line-soft bg-bg-main/50 px-3 py-2 text-sm leading-relaxed text-ink-soft">
            {event.query}
          </p>
        </section>
      )}
...
function MemoryIdSection({
  ...
  return (
    <section className="mt-5">
      <h4 className="m-0 mb-2 text-2xs font-semibold uppercase tracking-[0.08em] text-faint">{title} ({ids.length})</h4>
      <ul className="m-0 flex list-none flex-col gap-2 p-0">
        {ids.map((id) => (
          <li key={id} className="rounded-[8px] border border-line-soft bg-bg-main/50 px-3 py-2 text-sm leading-snug text-ink-soft">
            {onOpen ? (
