...
function ObservationRow({
  ...
  return (
    ...
    >
      <div className="text-sm leading-snug line-clamp-2">{obs.summary}</div>
          ...
          {editing ? (
            ...
            <>
              <DangerBtn onClick={() => void remove()} disabled={busy}>
                <Trash2 size={ICON.SM} strokeWidth={1.8} /> Delete
              </DangerBtn>
              <GhostBtn onClick={() => setEditing(true)} disabled={busy}>
                <Pencil size={ICON.SM} strokeWidth={1.8} /> Edit
              </GhostBtn>
            </>
          )}
        </>
      ...
      <ul className="flex flex-col gap-2">
        {facts.map((f) => {
          ...
          return (
            <li key={f.id} className="flex items-start gap-3">
              <span className="mt-[2px] text-2xs uppercase tracking-[0.06em] text-faint shrink-0 w-[80px]">
                {f.kind}
              </span>
                ...
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  ...
                  {sourceFocus && onOpenSource ? (
                    <button
                      ...
                      className="inline-flex min-w-0 items-center gap-1 text-left text-xs text-faint hover:text-ink-soft"
                      aria-label={`Open source for fact ${f.id}`}
                    >
                      <ExternalLink size={ICON.XS} strokeWidth={1.8} className="shrink-0" />
                      <span className="min-w-0 break-all">{factSourceSummary(f)}</span>
                      <span className="sr-only">Open source</span>
                    </button>
                  ) : (
                    <span className="text-xs text-faint">{factSourceSummary(f)}</span>
                  )}
                </div>
              </div>
            </li>
        ...
        {missing.map((id) => (
          <li key={`missing-${id}`} className="flex items-start gap-3 italic">
            ...
            <span className="text-sm text-faint">fact #{id} no longer available</span>
          </li>
        ))}
      </ul>
    </section>
