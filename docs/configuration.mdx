...
3. **Settings file** — `~/.ntrp/settings.json` (persisted by clients and server-side config updates)
4. **Auto-detection** — models resolved from available provider keys

## LLM providers

Set at least one provider API key, or connect the OpenAI Codex provider with browser sign-in from the desktop client. ntrp auto-selects models based on what's available.
...
Desktop clients store the key in local app storage and encrypt it with OS-backed storage when supported. The server stores only the salted hash.
