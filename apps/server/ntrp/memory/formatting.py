...
def format_memory_context(
    ...
    if query_observations:
        sections.append(
            (CONTEXTUAL_MEMORY_HEADER, [_format_observation_with_support(obs) for obs in query_observations])
        )
...
def format_memory_context_render(
    ...
    if query_facts and not query_observations:
        sections.append(
            (
                DIRECT_FACTS_HEADER,
                [_TrackedItem(text=f"- {fact.text}{_source_label(fact)}", fact_ids=(fact.id,)) for fact in query_facts],
            )
        )
