...
**Observations** are higher-level patterns synthesized from facts. "Alice has transitioned from mobile to backend engineering." They track their source facts (ordered by addition time), change history, evidence count, creator, and policy version. Legacy rows are marked as `created_by=legacy`, `policy_version=legacy`; new derived rows identify the automation policy that wrote them.

**Profile entries** are curated core-memory records. They are the only always-on profile surface and each entry carries direct provenance through `source_fact_ids` and/or `source_observation_ids`. Raw facts that merely mention `User` do not become profile.
...
5. Apply the decision: re-embed updated observations, record history entries, and stamp creator/policy metadata

The LLM is instructed to produce observations at a *higher abstraction level* than source facts - patterns, trajectories, inferences - not rephrases. Per-fact consolidation has a narrow create gate: it does not create a durable pattern from temporary facts or low-salience chat notes. Updates to existing observations still apply, and multi-fact temporal pattern creation is handled by the temporal pass.
...
ntrp/memory/
  ...
  observations.py     ObservationRepository (CRUD, search, merge)
    profile.py          ProfileRepository (curated core memory)
