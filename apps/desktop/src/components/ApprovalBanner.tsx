  ...
  return (
    ...
    >
      <header className="px-4 pt-3 pb-2 flex items-baseline gap-2">
        <h3 className="m-0 text-[15px] font-medium text-ink tracking-[-0.005em]">
          Approve <span className="font-mono">{toolName}</span>?
        </h3>
        {showBulk && (
          <span className="ml-auto shrink-0 text-[12px] text-faint tabular-nums">
            1 of {totalPending}
          </span>
        )}
      </header>
      ...
      {(path || previewLine) && (
        <dl className="px-4 pb-3 grid grid-cols-[max-content_minmax(0,1fr)] gap-x-6 gap-y-1 text-[13.5px]">
          {path && (
            <>
      ...
      <footer className="flex flex-wrap items-center gap-2 px-3 py-2 bg-surface-soft/35">
        {hasReviewable && (
          <button
            ...
            className="inline-flex items-center h-7 px-2.5 rounded-md text-[13px] text-muted hover:bg-surface hover:text-ink transition-colors"
          >
            Review
          </button>
        )}
        ...
        {showBulk && (
          <>
            ...
            <button
              ...
              className="inline-flex items-center h-7 px-3 rounded-md border border-line bg-surface text-[13px] text-ink-soft hover:bg-surface-soft hover:border-line-strong transition-colors"
            >
              Approve all
            </button>
        ...
        <button
          ...
          title="Approve (⌘↩)"
          className="inline-flex items-center gap-1.5 h-7 pl-3 pr-2 rounded-md bg-ink text-on-ink text-[13px] font-medium hover:opacity-90 transition-opacity"
        >
          Approve
          <span className="inline-flex items-center gap-0.5 opacity-70 text-[11.5px] font-mono leading-none">
            ⌘
