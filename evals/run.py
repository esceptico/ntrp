...
async def _run_one(
    ...
    async with sem:
        ...
        try:
            ...
            print(
                f"  {status}{cache_tag} | facts={len(result.facts)}/{result.total_facts} "
...
async def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="evals/cache",
        help="Cache dir for ingested DBs (default: evals/cache)",
    )
    parser.add_argument(
        "--no-cache",
        action="store_true",
        help="Disable DB caching, re-ingest every question",
    )
